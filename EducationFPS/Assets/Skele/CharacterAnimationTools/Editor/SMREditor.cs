...
namespace MH
{
    ...
    public class SMREditor : Editor
    {
        ...
        private static Texture2D ms_QuestionMark = null;
        ...
        private static bool ms_highDPIMode = false;
        ...
        void OnEnable()
        {
            if (!ms_inited)
            {
                ...
                ms_QuestionMark = AssetDatabase.LoadAssetAtPath(MH.Skele.CAT.ResPath.QuestionMark, typeof(Texture2D)) as Texture2D;
                Dbg.Assert(ms_QuestionMark != null, "SMREditor.sctor: failed to load questionMark at {0}", MH.Skele.CAT.ResPath.QuestionMark);
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (ms_bEditing)
            {
                ...
                GUILayout.BeginHorizontal();
                {
                    ...
                    msg = ms_highDPIMode ? "Use Low DPI Mode" : "Use High DPI Mode";
                    if( EUtil.Button(msg, "Use High DPI mode on Mac Retina screen if there's issue displaying GUI", Color.white) )
                    {
                        ms_highDPIMode = !ms_highDPIMode;
                        _SceneRepaintAll();
                    }
                }
                GUILayout.EndHorizontal();
            ...
            GUILayout.BeginHorizontal();
            {
                ...
                EUtil.PushBackgroundColor(Color.blue);
                if (GUILayout.Button(new GUIContent(ms_QuestionMark, "Jump to the online manual's F.A.Q section"), GUILayout.Width(24f), GUILayout.Height(24f)))
                {
                    Application.OpenURL("http://www.tmpxyz.com/Skele/docs/84-utility/81-f-a-q");
                }
        ...
        private static float _GetScreenWidth()
        {
            if( ms_highDPIMode )
            {
        ...
        private static float _GetScreenHeight()
        {
            if( ms_highDPIMode )
            {
...
");
        ...
        private static void _DrawSceneGUI_AnimHelper()
        {
            ...
            {
                Rect rect = new Rect(
                    _GetScreenWidth() - WND_WIDTH,
                    _GetScreenHeight() - WND_HEIGHT - ANIM_HELPER_WND_HEIGHT - 1,
                    WND_WIDTH,
        ...
        {
            Rect rect = new Rect(_GetScreenWidth() - WND_WIDTH, _GetScreenHeight() - WND_HEIGHT, WND_WIDTH, WND_HEIGHT);
            if (ms_Background != null)
                GUI.DrawTexture(rect, ms_Background);
        ...
        {
            // Main IK UI 
            Rect mainUIRect = new Rect(_GetScreenWidth() - WND_WIDTH, _GetScreenHeight() - WND_HEIGHT, WND_WIDTH, WND_HEIGHT);
            if (ms_Background != null)
                ...
                GUILayout.BeginArea(rect);
                {
                    ...
                    GUILayout.BeginHorizontal(GUILayout.Width(WND_WIDTH - 20));
                    {
                        if (EUtil.Button("Save", "Save the poses to file"))
                        {
                            ...
                            {
                                string defPath = _GetPoseMgrSaveLoadPath();
                                string pathName = EditorUtility.SaveFilePanel("Save_Pose_File", defPath, "defaultName", "pose");
                                if (!string.IsNullOrEmpty(pathName))
                                {
                                    ms_PoseSet.FileName = pathName;
                                    ms_PoseSet.Save();
                                    _SavePoseMgrSaveLoadPath(pathName);
                                }
                            }
                            else
                            {
                        ...
                        {
                            string defPath = _GetPoseMgrSaveLoadPath();
                            string pathName = EditorUtility.OpenFilePanel("Open_Pose_File", defPath, "pose");
                            if (!string.IsNullOrEmpty(pathName))
                            {
        ...
        /// <summary>
        /// retrieve poseMgr's save/load path from EditorPref
        /// </summary>
        private static string _GetPoseMgrSaveLoadPath()
        {
            string path = EditorPrefs.GetString(EDITOR_PREF_KEY_POSEMGR_SLPATH, "");
            return path;
        }

        /// <summary>
        /// given a path "A/B/C.pose", save "A/B" to EditorPrefs
        /// </summary>
        private static void _SavePoseMgrSaveLoadPath(string path)
        {
            if (!string.IsNullOrEmpty(path))
            {
                string dir = System.IO.Path.GetDirectoryName(path);
                EditorPrefs.SetString(EDITOR_PREF_KEY_POSEMGR_SLPATH, dir);
            }
        }
        ...
        public const string EDITOR_PREF_KEY_POSEMGR_SLPATH = "__MH_Skele_PoseMgrSaveLoadPath";
