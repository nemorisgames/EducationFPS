...
public class AMCameraFade : MonoBehaviour {
	...
	// hide in inspector
	[HideInInspector] public int keepAlives = 0;
	[HideInInspector] public bool keepAliveColor = false;
	[HideInInspector] public bool keepAlivePreview = false;
	[HideInInspector] public bool keepAliveAwake = true;
	[HideInInspector] public int mode = 0;
	[HideInInspector] public float percent;
	/*[HideInInspector]*/ public float value;
	[HideInInspector] public bool useRenderTexture = false;
	[HideInInspector] public Texture2D tex2d;
	[HideInInspector] public RenderTexture _tex;
	[HideInInspector] private RenderTexture tex {
		get {
			return getRenderTexture();
		}
	}
	[HideInInspector] public float[] r;
	[HideInInspector] public Camera renderTextureCamera;
	[HideInInspector] public bool hasColorTex = false;
	[HideInInspector] public bool hasColorBG = false;
	[HideInInspector] public Color colorTex;
	[HideInInspector] public Color colorBG;
	[HideInInspector] public int width = 0;
	[HideInInspector] public int height = 0;
	[HideInInspector] public bool placeholder = false;
	[HideInInspector] public Texture2D irisShape;
	[HideInInspector] public Texture background;
	[HideInInspector] public int cachedStillFrame = 0;
	[HideInInspector] public bool isReset = false;
	[HideInInspector] public bool shouldUpdateStill = false;
	[HideInInspector] public bool shouldUpdateRenderTexture = false;
	...
	void OnGUI () {
        ...
        //if (e.type == EventType.KeyDown && e.keyCode == KeyCode.F12) isKeyDown = true;
        //if (isKeyDown && e.type == EventType.Repaint)
        //    RTUtil.WriteRTToFile(tex, "Assets/rt0.png");
        ...
        //if (isKeyDown && e.type == EventType.Repaint)
        //    RTUtil.WriteRTToFile(tex, "Assets/rt1.png");
	...
	public static void processAlpha(Rect rect, Texture tex, float value) {
        ...
        GUI.DrawTexture(rect, tex, ScaleMode.StretchToFill, false); //disable alphaBlend in U5 too  http://forum.unity3d.com/threads/render-texture-alpha.2065/
	}
    ...
    public static void processIrisShape(Rect rect, Texture tex, bool shrink, float value, float percent, float focalX, float focalY, Texture shape, float maxScale, float pivotX, float pivotY, int rotateAmount, bool easeRotation, Event editorEvent)
    {
        if (editorEvent.type.Equals(EventType.Repaint))
        {
            focalY = 1f - focalY;
            float scale = (shrink ? value : 1f - value) * maxScale;
            if (scale <= 0.01f) scale = 0.01f; //prevent divide 0

            if (scale <= 0.04f) matIris.SetFloat("_Cutoff", 0.8f);// set cutoff
            else matIris.SetFloat("_Cutoff", 0.1f);

            float aspect = rect.width / rect.height; //aspect, use this to keep circle from becoming ellipse

            Vector3 _scale = new Vector3(1f/scale*aspect, 1f/scale, 1f);
            Vector3 _position = new Vector3(focalX, focalY, 0);
            float value2 = ((shrink ? 0f : 1f) - value);
            ...
            // prepare matrices
            Matrix4x4 mtxRT = Matrix4x4.TRS(pivot, Quaternion.identity, Vector3.one);
            Matrix4x4 mtxR = Matrix4x4.TRS(Vector3.zero, Quaternion.Euler(0, 0, _rotate), Vector3.one);
            Matrix4x4 mtxRInvT = Matrix4x4.TRS(-pivot, Quaternion.identity, Vector3.one);
            Matrix4x4 mtxS = Matrix4x4.TRS(Vector3.zero, Quaternion.identity, _scale);
            Matrix4x4 mtxSInvT = Matrix4x4.TRS(-_position, Quaternion.identity, Vector3.one);
            Matrix4x4 mtxST = Matrix4x4.TRS(_position, Quaternion.identity, Vector3.one);

            // set matrix
            var mtx = mtxRT * mtxR * mtxRInvT * mtxST * mtxS * mtxSInvT;
            matIris.SetMatrix("_Matrix", mtx);
