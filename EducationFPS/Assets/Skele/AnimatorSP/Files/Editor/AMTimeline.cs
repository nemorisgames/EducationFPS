...
public class AMTimeline : EditorWindow {
    ...
    [MenuItem("Window/Skele/Animator SP : Timeline Editor &#w")]
    static void Init () {
        if (window != null)
        {
            window.Close();
        ...
        {
            window = (AMTimeline)EditorWindow.GetWindow(typeof(AMTimeline));
        }
    }

    /// <summary>
    /// reset the AMTimeline with pass-in AnimatorData
    /// </summary>
    public static void ResetWithAnimatorData(AnimatorData paramData)
    {
        if (paramData == null)
            return;
        if (window != null)
        {
            window.Close();
            ms_loadAtStart = false;
        }
        Init();
        window.aData = paramData;
    }
    ...
    private static bool ms_loadAtStart = true;

    private static GameObject ms_dataHolderGO = null; //these two are a group to handle both close window & recompile/run switch
    private GameObject m_dataHolderGO = null; 

    private AnimatorData m_tmpDataHolder = null;
	...
	public AnimatorData aData {
		...
		set {
            if ((!_aData) || // this is to handle the missing ref case
                _aData != value  ) 
            {
                if (_aData != null)
                {
                    if( _aData.getCurrentTakeValue() >= 0 && _aData.getTakeCount() > 0)
                        _aData.getCurrentTake().SaveAsset(_aData);
                    _aData.evtBefChangeCurTake -= _OnBeforeChangeCurTake;
                }
                ...
                if (_aData != null)
                {
                    _SetDataHolderGO(_aData.gameObject);

                    _aData.evtBefChangeCurTake += _OnBeforeChangeCurTake;

                    // remove the null takes, ensure the validity of takes list
                    for (int i = _aData.getTakeCount() - 1; i >= 0; --i)
                    {
                        var oneTake = _aData.takes[i];
                        if (!oneTake || oneTake == null) // null ref or broken 
                        {
                            _aData.removeTake(i);
                        }
                    }

                    // set the editTimeRoot
                    RebindOption.editTimeRoot = _aData.transform;
	...
	// textures
	private Texture tex_cursor_zoomin = null;
	private Texture tex_cursor_zoomout = null;
	private Texture tex_cursor_zoom_blank = null;
	private Texture tex_cursor_zoom = null;
	private Texture tex_cursor_grab = null;
	private Texture tex_icon_track = null;
	private Texture tex_icon_track_hover = null;
	private Texture tex_icon_group_closed = null;
	private Texture tex_icon_group_open = null;
	private Texture tex_icon_group_hover = null;
	private Texture tex_element_position = null;
    private Texture texFrKey = null;
	private Texture texFrSet = null;
	//private Texture texFrU = null;
	//private Texture texFrM = null;
	//private Texture texFrUS = null;
	//private Texture texFrMS = null;
	//private Texture texFrUG = null;
	private Texture texKeyBirdsEye = null;
	private Texture texIndLine = null;
	private Texture texIndHead = null;
	private Texture texProperties = null;	
	private Texture texRightArrow = null;// inspector right arrow
	private Texture texLeftArrow = null;	// inspector left arrow
	private Texture[] texInterpl = null;
	private Texture texBoxBorder = null;
	private Texture texBoxRed = null;
    private Texture texBoxBlue = null;
	private Texture texBoxLightBlue = null;
	private Texture texBoxDarkBlue = null;
	private Texture texBoxGreen = null;
	private Texture texBoxPink = null;
	private Texture texBoxYellow = null;
	private Texture texBoxOrange = null;
	private Texture texBoxPurple = null;
	private Texture texIconTranslation = null;
	private Texture texIconRotation = null;
	private Texture texIconAnimation = null;
    private Texture texIconAnimator = null;
	private Texture texIconAudio = null;
	private Texture texIconProperty = null;
	private Texture texIconEvent = null;
	private Texture texIconOrientation = null;
    private Texture texIconScale = null;
	private Texture texIconCameraSwitcher = null;

    private Texture texTakeIcon = null;
    ...
    // tmp vars
    private Vector2 tmp_scrollpos_animator = Vector2.zero;

	#endregion
	
	#region Main

    public static AnimatorData GetAnimatorData()
    {
        if (window != null)
            return window.aData;
        else
	...
	void OnEnable() {
        ...
        EUtil.SetEditorWindowTitle(this, "AnimatorSP", texTakeIcon);
        this.minSize = new Vector2(width_track + width_playback_controls + width_inspector_open + 70f, 190f);

        if (aData != null)
        {
            _SetDataHolderGO(aData.gameObject);
            //_OnFoundAData();
        }
		//this.wantsMouseMove = true;
		// find component
        if (ms_loadAtStart && !EditorApplication.isPlayingOrWillChangePlaymode)
        {
            GameObject go = _GetDataHolderGO() ?? GameObject.Find("AnimatorData");
            if (go)
            {
        ...
        // Undo.Modification callback
        Undo.postprocessModifications += _OnPostprocessModifications;
	}
	...
	void OnDisable() {
		...
		if(aData) {
            ...
            aData = null; //will trigger saving current take
		}
        ...
        // disable the RebindTr.editTimeRoot
        RebindOption.editTimeRoot = null;
	}
	void Update() {
		isPlayMode = EditorApplication.isPlayingOrWillChangePlaymode;
		
		if(isPlayMode) return;
        if (!aData || aData.takes.Count == 0) return;
		// drag logic
		if(!isPlaying) {
	...
	void OnGUI() {
		...
		if(!aData) {
            ...
            GUILayout.BeginHorizontal();
			    ...
			    if(GUILayout.Button (new GUIContent("Add New Instance", "add new AnimatorData GameObject"))) {
				    // create component
				    var go = new GameObject("AnimatorData");
            ...
            GUILayout.BeginHorizontal();
                GUILayout.Space(100f);
                if (GUILayout.Button(new GUIContent("AutoFind Component", "Look for AnimatorData instance in current scene")))
                {
				    // create component
                    aData = GameObject.FindObjectOfType<AnimatorData>();
                    if (aData != null)
                    {
                        aData.isAnimatorOpen = true;
                    ...
                    {
                        ShowNotification(new GUIContent("Not Found AnimatorData in current scene"));
                    }
			    }
                GUILayout.Space(100f);
            GUILayout.EndHorizontal();
            ...
            GUILayout.BeginHorizontal();
                GUILayout.Space(100f);
                m_tmpDataHolder = (AnimatorData)EditorGUILayout.ObjectField(new GUIContent("Select From Scene", "Select AnimatorData from current scene"), m_tmpDataHolder, typeof(AnimatorData), true);
                if( GUI.changed && m_tmpDataHolder != null  )
                {
                    if (EditorUtility.IsPersistent(m_tmpDataHolder))
                    {
                        ShowNotification(new GUIContent("Only Accept Scene Object"));
                        m_tmpDataHolder = null;
                    }
                }
                if (m_tmpDataHolder != null && GUILayout.Button("Confirm"))
                {
                    aData = m_tmpDataHolder;
                    m_tmpDataHolder = null;
                    aData.isAnimatorOpen = true;
		...
		#region no take
		//when no take in the AnimatorData, must "create new take" or "load existing take"
        if (!_OnGUI_NoTake())
            return; // if return false, means no take in aData, and must stop onGUI here
		...
		if(!oData.ignoreMinSize && (position.width<width_window_minimum)) {
			...
			GUILayout.BeginHorizontal();
				if(GUILayout.Button("Ignore (Not Recommended)")) {
                    AMUtil.recordObject(oData, "Ignore Min. Window Size");
				...
				if(GUILayout.Button("Resize")) {
                    AMUtil.recordObject(this, "Window Resize");
		...
		bool showCursor = true;
		 if(!isRenamingTake && isRenamingGroup >= 0 && isRenamingTrack <= -1 && (dragType==(int)DragType.CursorHand || (!cursorZoom && (isSpaceBarDown||isMMBDown) && EditorWindow.mouseOverWindow==this))) {
			cursorHand = true;
			showCursor = false;
		...
		if(Misc.IsShowCursor() != showCursor) {
			Misc.ShowCursor(showCursor);
		}
		...
		#region power btn
        Rect rectBtnPower = new Rect(margin, 0, 30f, height_button_delete);
        if (GUI.Button(rectBtnPower, new GUIContent(getSkinTextureStyleState("power").background, "Reset"), EditorStyles.toolbarButton))
        {
            if (EditorUtility.DisplayDialog("Reset", "Animator_SP Timeline Editor will be RESET IMMEDIATELY!\n\nAre you sure you want to do that?", "Yes", "No"))
            {
                Close();
                ms_loadAtStart = false;
		...
		Rect rectBtnOptions = new Rect(margin+30f,0f,30f,height_button_delete);
		if(GUI.Button(rectBtnOptions, new GUIContent(getSkinTextureStyleState("setting").background, "Settings"), EditorStyles.toolbarButton)) {
            EditorGUIUtility.PingObject(_GetDataHolderGO());
            if (AMOptions.window != null)
            {
                AMOptions.window.Close();
            }
            else
            {
		...
		if(isRenamingTake) {
			...
			rectRenameTake.y += 3f;
            AMUtil.regUndoSelectedTake(aData, "Rename Take");
			string takeName = GUI.TextField(rectRenameTake,aData.getCurrentTake().name,EditorStyles.toolbarTextField);
            aData.getCurrentTake().name = takeName;
		...
		#region import take button
        Rect rectBtnImportTake = new Rect(rectBtnRenameTake.x + rectBtnRenameTake.width + margin, rectBtnRenameTake.y, width_button_delete, height_button_delete);
        if (GUI.Button(rectBtnImportTake, new GUIContent(getSkinTextureStyleState("import").background, "Import Take"), EditorStyles.toolbarButton))
        {
            _ShowImportTakeWindow(rectBtnImportTake);
        }
		#endregion import take button

		#region remove take button
        Rect rectBtnRemoveTake = new Rect(rectBtnImportTake.x + rectBtnImportTake.width + margin, rectBtnRenameTake.y, width_button_delete, height_button_delete);
		if(GUI.Button(rectBtnRemoveTake,new GUIContent("","Remove Take"),/*GUI.skin.GetStyle("ButtonImage")*/EditorStyles.toolbarButton)) {
			if((EditorUtility.DisplayDialog("Remove Take","Are you sure you want to remove take '"+aData.getCurrentTake().name+"'?","Remove","Cancel"))) {
                //registerUndo("Delete Take");

                //aData.deleteCurrentTake();
                aData.removeCurrentTake();
				AMCodeView.resetTrackDictionary();
				// save data
				EditorUtility.SetDirty(aData);
				setDirtyTakes(aData.takes);
                ...
                if (aData.takes.Count == 0)
                    return; //when no take left, break out the OnGUI
			}
		}
		if(!GUI.enabled) GUI.color = new Color(GUI.color.r,GUI.color.g,GUI.color.b,0.25f);
		GUI.DrawTexture(new Rect(rectBtnRemoveTake.x+(rectBtnRemoveTake.height-10f)/2f,rectBtnRemoveTake.y+(rectBtnRemoveTake.width-10f)/2f-2f,10f,10f),(getSkinTextureStyleState((GUI.enabled && rectBtnRemoveTake.Contains(e.mousePosition) ? "delete_hover" : "delete")).background));
		if(rectBtnRemoveTake.Contains(e.mousePosition) && mouseOverElement == (int)ElementType.None) mouseOverElement = (int)ElementType.Button;
		if(GUI.color.a < 1f) GUI.color = new Color(GUI.color.r,GUI.color.g,GUI.color.b,1f);
		#endregion
		#region create new take after deleting
        //if(aData.getCurrentTakeValue() == aData.takes.Count || aData.getCurrentTakeValue() < 0) {
        //    //registerUndo("New Take");
        //    AMUtil.recordObject(aData, "New Take");
        //    cancelTextEditting();
        //    aData.addTake ();
        //    // save data
        //    EditorUtility.SetDirty(aData);
		...
		#region play on start button
			Rect rectBtnPlayOnStart = new Rect(rectBtnRemoveTake.x+rectBtnRemoveTake.width+margin,rectBtnRemoveTake.y,width_button_delete,height_button_delete);
		...
		#endregion
		#region settings
		Rect rectLabelSettings = new Rect(rectBtnPlayOnStart.x+rectBtnPlayOnStart.width+margin,rectBtnPlayOnStart.y,180f,rectLabelCurrentTake.height);
		...
		GUI.BeginGroup(rectAreaPlaybackControls);
			...
			if(GUI.Button (rectBtnNewGroup,new GUIContent("","New Group"),"label")) {
                ...
                AMUtil.recordObject(this, "New Group");
                AMUtil.recordObject(aData.getCurrentTake(), "New Group");
        ...
        else
        {
            ...
            if (rectHScrollbarThumb.width < rectResizeHScrollbarLeft.width * 2f)
            {
                ...
                rectResizeHScrollbarRight = new Rect(rectHScrollbarThumb.x + rectHScrollbarThumb.width / 2f, rectResizeHScrollbarRight.y, rectResizeHScrollbarLeft.width, rectResizeHScrollbarRight.height);
            }
            ...
            {
                Mathf.Clamp((int)GUI.HorizontalScrollbar(rectHScrollbar, (float)aData.getCurrentTake().startFrame, (int)numFramesToRender - 1f, 1f, aData.getCurrentTake().numFrames), 1f, aData.getCurrentTake().numFrames);
            }
        ...
        if (tempGO != null)
        {
            ...
            foreach( var o in DragAndDrop.objectReferences)
            {
                if( o is GameObject )
                {
                    objects_window.Add(o as GameObject);
                }
            }
	...
	void OnUndoRedo() {
        ...
        if (aData == null || aData.gameObject == null)
            aData = null;
	...
	void OnPlayMode() {
		...
		if(justHitPlay) {
			if(aData) {
                //save current take
                if (aData.getTakeCount() > 0)
                    aData.getCurrentTake().SaveAsset(aData, AssetDatabase.GetAssetPath(aData.getCurrentTake()));
		...
		// exit playmode
            ...
            //Dbg.Log("isPlaying: {0}, isPlayingOrGoing: {1}", EditorApplication.isPlaying, EditorApplication.isPlayingOrWillChangePlaymode); //could be false, true

			// recheck for component
            //GameObject go = GameObject.Find ("AnimatorData");
            GameObject go = _GetDataHolderGO(); 
			if(go) {
				aData = (AnimatorData) go.GetComponent<AnimatorData>();
                if (aData) {
    ...
    //{
    //    //AMUtil.recordObject(window, name);
    //    //AMUtil.recordObject(window.aData, name);
    //    //var takes = window.aData.takes;
    //    //foreach (var oneTake in takes)
    //    //{
	...
	public static void loadSkin(AMOptionsFile oData, ref GUISkin _skin, ref string skinName, Rect position) {		
		if(_skin == null || skinName == null || skinName != oData.skin/*global_skin*/) {
			//
			_skin = (GUISkin) Resources.Load(oData.skin/*global_skin*/);
	...
	public static GUIStyleState getSkinTextureStyleState(string name) {
        ...
        if(name == "import")            return GUI.skin.GetStyle("Textures_3").active;
        if(name == "power")             return GUI.skin.GetStyle("Textures_3").focused;
        if(name == "setting")           return GUI.skin.GetStyle("Textures_3").onNormal;
	...
	bool showTrack(AMTrack _track, int t, int group_level, ref float track_y, ref bool isAnyTrackFoldedOut, ref float height_group_elements, Event e,Vector2 scrollViewBounds) {
		...
		if(isRenamingTrack != t) {
			...
			if(GUI.Button(new Rect(rectTrack.x+15f,rectTrack.y,rectTrack.width-15f-20f,rectTrack.height),"","label")) { //minus the enable toggle width too
					timelineSelectTrack(t);
		...
		if(!_track.foldout) {
			if(rectTrack.width > 4f) {
				...
				GUI.BeginGroup(rectTrack,GUI.skin.GetStyle(strStyle));
					...
					GUI.Label(new Rect(17f+12f+2f,0f,rectTrack.width-(12f+15f+2f+20f),height_track_foldin),_track.name);
                    _track.enabled = GUI.Toggle(new Rect(rectTrack.width - 20f, 0f, 20f, 15f), _track.enabled, new GUIContent("", "enable?"));
				GUI.EndGroup();
				// draw foldout
			...
			if(rectTrack.width > 4f) {
				...
				GUI.BeginGroup(rectTrack,GUI.skin.GetStyle(strStyle));
					...
					if(isRenamingTrack == t) {
						...
						GUI.Label(new Rect(15f,0f,rectTrack.width-15f-20f,20f),_track.name);
                        _track.enabled = GUI.Toggle(new Rect(rectTrack.width - 20f, 0f, 20f, 15f), _track.enabled, new GUIContent("", "enable?"));
					}
					// track type
	...
	void showFrames(AMTrack _track, ref float track_y, Event e, bool birdseye, Vector2 scrollViewBounds) {
		...
		GUI.BeginGroup(rectFrames);
			...
			} else if(aData.getCurrentTake().contextSelection!=null && aData.getCurrentTake().contextSelection.Count > 0 && /*do not show single frame selection in birdseye*/!(birdseye && aData.getCurrentTake().contextSelection.Count == 2 && aData.getCurrentTake().contextSelection[0] == aData.getCurrentTake().contextSelection[1])) {
		...
		if(!oData.disableTimelineActions && _track.foldout) {
			...
			if(_track is AMAnimationTrack || _track is AMAnimatorTrack || _track is AMAudioTrack) drawEachAction = true;	// draw each action with seperate textures and buttons for these tracks
			...
			if(!drawEachAction) {
				if(_track.cache.Count > 0) {
					if(_track is AMTranslationTrack && _track.cache.Count > 1) {
						...
						cached_action_endFrame = _track.cache[_track.cache.Count-1].startFrame;
						texBox = texBoxOrange;
                    } else if (_track is AMScaleTrack && _track.cache.Count > 1) {
                        cached_action_startFrame = _track.cache[0].startFrame;
                        cached_action_endFrame = _track.cache[_track.cache.Count - 1].startFrame;
                        texBox = texBoxBlue;
			...
			for(int i=0;i<_track.cache.Count;i++) {
                ...
                } else if(_track is AMAnimatorTrack){
                    action_startFrame = _track.cache[i].startFrame;
                    if (i < _track.cache.Count - 1)
                        action_endFrame = (_track.cache[i + 1] as AMAnimatorAction).startFrame;
                    else
                        action_endFrame = aData.getCurrentTake().numFrames;
                ...
                else if (_track is AMAnimatorTrack) texBox = texBoxRed;
                ...
                else if (_track is AMScaleTrack) texBox = texBoxBlue;
	...
	void showInspectorPropertiesFor(Rect rect, int _track, int _frame, Event e) {		
        ...
        if (sTrack is AMScaleTrack)
        {
            var oneTrack = sTrack as AMScaleTrack;
            AMScaleKey sKey = (AMScaleKey)oneTrack.getKeyOnFrame(_frame);
            Rect rectScale = new Rect(0f, start_y, width_inspector - margin, 40f);
            // quaternion
            if (sKey.setScale(EditorGUI.Vector3Field(rectScale, "Scale", sKey.scale)))
            {
                // update cache when modifying varaibles
                sTrack.updateCache();
                AMCodeView.refresh();
		...
		#endregion scale inspector
		#region animator inspector
        if (sTrack is AMAnimatorTrack) {
            EUtil.PushLabelWidth(80f);
            var oneTrack = sTrack as AMAnimatorTrack;
            var ator = oneTrack.animator;
            if (ator != null)
            {
                AMAnimatorKey aKey = (AMAnimatorKey)oneTrack.getKeyOnFrame(_frame);
                string[] animActionOptions = new string[] { "jump state", "set param" };
                var infos = aKey.infos;

                AMUtil.recordObject(aKey, "inspector for animator");
                Rect areaRc = new Rect(0, start_y, width_inspector - margin, rect.height - start_y);
                GUILayout.BeginArea(areaRc);
                tmp_scrollpos_animator = EditorGUILayout.BeginScrollView(tmp_scrollpos_animator);
                for (int kIdx = 0; kIdx < infos.Count; ++kIdx)
                {
                    EditorGUI.BeginChangeCheck();
                    var oneInfo = infos[kIdx];
                    oneInfo.animAction = (AnimatorKeyInfo.eAction)EditorGUILayout.Popup("action", (int)oneInfo.animAction, animActionOptions);
                    if (oneInfo.animAction == AnimatorKeyInfo.eAction.Switch)
                    {
                        // target state
                        oneInfo.targetState = EUtil.AnimatorStatePopup(new GUIContent("Target State", "will use Play/CrossFade to directly jump to this state"), ator, oneInfo.targetState);

                        // fade time
                        oneInfo.fadeTime = EditorGUILayout.FloatField(new GUIContent("Fade time", "will use cross fade if greater than 0"), oneInfo.fadeTime);
                    }
                    else
                    {// transit
                        oneInfo.paramName = EUtil.AnimatorParamPopup(ator, oneInfo.paramName);
                        AnimatorControllerParameter aparam = EUtil.AnimatorGetParameter(ator, oneInfo.paramName);

                        switch (aparam.type)
                        {
                            case AnimatorControllerParameterType.Bool:
                                oneInfo.animParam = AnimatorKeyInfo.eParam.Bool;
                                oneInfo.vBool = EditorGUILayout.Toggle("bool value", oneInfo.vBool);
                                break;
                            case AnimatorControllerParameterType.Float:
                                oneInfo.animParam = AnimatorKeyInfo.eParam.Float;
                                oneInfo.vFloat = EditorGUILayout.FloatField("float value", oneInfo.vFloat);
                                break;
                            case AnimatorControllerParameterType.Int:
                                oneInfo.animParam = AnimatorKeyInfo.eParam.Integer;
                                oneInfo.vInt = EditorGUILayout.IntField("int value", oneInfo.vInt);
                                break;
                            case AnimatorControllerParameterType.Trigger:
                                oneInfo.animParam = AnimatorKeyInfo.eParam.Trigger;
                                EditorGUILayout.LabelField("set trigger \"" + oneInfo.paramName + "\"");
                                break;
                            default:
                                Dbg.LogErr("unexpected animator parameter type: {0}", aparam.type);
                                break;
                        }
                    }

                    if (EUtil.Button("Delete Item", Color.red))
                    {
                        infos.RemoveAt(kIdx);
                        --kIdx;
                    }
                    ...
                    {
                        _OnTimelineInspectorChange(sTrack);
                    }

                }// for(int kidx = 0 ... )

                if (EUtil.Button("Add Item", Color.green))
                {
                    infos.Add(new AnimatorKeyInfo());
                    _OnTimelineInspectorChange(sTrack);
                }
		...
		if(sTrack is AMPropertyTrack) {
            ...
            {
                EUtil.PushLabelWidth(60f);
                rectField.height = 20f;
                if(pKey.setValue(EditorGUI.TextField(rectField, propertyLabel, pKey.str))){
			...
			if((sTrack as AMPropertyTrack).valueType != (int)AMPropertyTrack.ValueType.MorphChannels &&
                (sTrack as AMPropertyTrack).valueType != (int)AMPropertyTrack.ValueType.String && 
                pKey != (sTrack as AMPropertyTrack).keys[(sTrack as AMPropertyTrack).keys.Count-1]) {
    ...
    private void _OnTimelineInspectorChange(AMTrack sTrack)
    {
        sTrack.updateCache();
        AMCodeView.refresh();
        // preview new position
	...
	bool showFieldFor(Rect rect, string id, string name, AMEventParameter parameter, Type t, int level, ref float height_field) {
		...
		if(t.IsArray) {
			...
			if(parameter.setObject(EditorGUI.ObjectField(rectObjectField,parameter.val_obj,typeof(GameObject),bAllowSceneObject))) saveChanges = true;
			GUI.skin = skin;
		    //EditorGUIUtility.LookLikeControls();
            EditorGUIUtility.labelWidth=0f;
            ...
            EditorGUIUtility.labelWidth=0f;
			// field
			if(t == typeof(Transform)) { if(parameter.setObject(EditorGUI.ObjectField(rectObjectField,parameter.val_obj,typeof(Transform),bAllowSceneObject))) saveChanges = true; }
			else if(t == typeof(MeshFilter)) { if(parameter.setObject(EditorGUI.ObjectField(rectObjectField,parameter.val_obj,typeof(MeshFilter),bAllowSceneObject))) saveChanges = true; }
			else if(t == typeof(TextMesh)) { if(parameter.setObject(EditorGUI.ObjectField(rectObjectField,parameter.val_obj,typeof(TextMesh),bAllowSceneObject))) saveChanges = true; }
			else if(t == typeof(MeshRenderer)) { if(parameter.setObject(EditorGUI.ObjectField(rectObjectField,parameter.val_obj,typeof(MeshRenderer),bAllowSceneObject))) saveChanges = true; }
			//else if(t == typeof(ParticleSystem)) { if(parameter.setObject(EditorGUI.ObjectField(rectObjectField,parameter.val_obj,typeof(ParticleSystem),bAllowSceneObject))) saveChanges = true; }
			else if(t == typeof(TrailRenderer)) { if(parameter.setObject(EditorGUI.ObjectField(rectObjectField,parameter.val_obj,typeof(TrailRenderer),bAllowSceneObject))) saveChanges = true; }
			else if(t == typeof(LineRenderer)) { if(parameter.setObject(EditorGUI.ObjectField(rectObjectField,parameter.val_obj,typeof(LineRenderer),bAllowSceneObject))) saveChanges = true; }
			else if(t == typeof(LensFlare)) { if(parameter.setObject(EditorGUI.ObjectField(rectObjectField,parameter.val_obj,typeof(LensFlare),bAllowSceneObject))) saveChanges = true; }
			// halo
			else if(t == typeof(Projector)) { if(parameter.setObject(EditorGUI.ObjectField(rectObjectField,parameter.val_obj,typeof(Projector),bAllowSceneObject))) saveChanges = true; }
			else if(t == typeof(Rigidbody)) { if(parameter.setObject(EditorGUI.ObjectField(rectObjectField,parameter.val_obj,typeof(Rigidbody),bAllowSceneObject))) saveChanges = true; }
			else if(t == typeof(CharacterController)) { if(parameter.setObject(EditorGUI.ObjectField(rectObjectField,parameter.val_obj,typeof(CharacterController),bAllowSceneObject))) saveChanges = true; }
			else if(t == typeof(BoxCollider)) { if(parameter.setObject(EditorGUI.ObjectField(rectObjectField,parameter.val_obj,typeof(BoxCollider),bAllowSceneObject))) saveChanges = true; }
			else if(t == typeof(SphereCollider)) { if(parameter.setObject(EditorGUI.ObjectField(rectObjectField,parameter.val_obj,typeof(SphereCollider),bAllowSceneObject))) saveChanges = true; }
			else if(t == typeof(CapsuleCollider)) { if(parameter.setObject(EditorGUI.ObjectField(rectObjectField,parameter.val_obj,typeof(CapsuleCollider),bAllowSceneObject))) saveChanges = true; }
			else if(t == typeof(MeshCollider)) { if(parameter.setObject(EditorGUI.ObjectField(rectObjectField,parameter.val_obj,typeof(MeshCollider),bAllowSceneObject))) saveChanges = true; }
			else if(t == typeof(WheelCollider)) { if(parameter.setObject(EditorGUI.ObjectField(rectObjectField,parameter.val_obj,typeof(WheelCollider),bAllowSceneObject))) saveChanges = true; }
			else if(t == typeof(TerrainCollider)) { if(parameter.setObject(EditorGUI.ObjectField(rectObjectField,parameter.val_obj,typeof(TerrainCollider),bAllowSceneObject))) saveChanges = true; }
            //else if(t == typeof(InteractiveCloth)) { if(parameter.setObject(EditorGUI.ObjectField(rectObjectField,parameter.val_obj,typeof(InteractiveCloth),bAllowSceneObject))) saveChanges = true; }
            //else if(t == typeof(SkinnedCloth)) { if(parameter.setObject(EditorGUI.ObjectField(rectObjectField,parameter.val_obj,typeof(SkinnedCloth),bAllowSceneObject))) saveChanges = true; }
            //else if(t == typeof(ClothRenderer)) { if(parameter.setObject(EditorGUI.ObjectField(rectObjectField,parameter.val_obj,typeof(ClothRenderer),bAllowSceneObject))) saveChanges = true; }
			else if(t == typeof(HingeJoint)){ if(parameter.setObject(EditorGUI.ObjectField(rectObjectField,parameter.val_obj,typeof(HingeJoint),bAllowSceneObject))) saveChanges = true; }
			else if(t == typeof(FixedJoint)) { if(parameter.setObject(EditorGUI.ObjectField(rectObjectField,parameter.val_obj,typeof(FixedJoint),bAllowSceneObject))) saveChanges = true; }
			else if(t == typeof(SpringJoint)) { if(parameter.setObject(EditorGUI.ObjectField(rectObjectField,parameter.val_obj,typeof(SpringJoint),bAllowSceneObject))) saveChanges = true; }
			else if(t == typeof(CharacterJoint)) { if(parameter.setObject(EditorGUI.ObjectField(rectObjectField,parameter.val_obj,typeof(CharacterJoint),bAllowSceneObject))) saveChanges = true; }
			else if(t == typeof(ConfigurableJoint)) { if(parameter.setObject(EditorGUI.ObjectField(rectObjectField,parameter.val_obj,typeof(ConfigurableJoint),bAllowSceneObject))) saveChanges = true; }
			else if(t == typeof(ConstantForce)) { if(parameter.setObject(EditorGUI.ObjectField(rectObjectField,parameter.val_obj,typeof(ConstantForce),bAllowSceneObject))) saveChanges = true; }
			//else if(t == typeof(NavMeshAgent)) { if(parameter.setObject(EditorGUI.ObjectField(rectObjectField,parameter.val_obj,typeof(NavMeshAgent),bAllowSceneObject))) saveChanges = true; }
			//else if(t == typeof(OffMeshLink)) { if(parameter.setObject(EditorGUI.ObjectField(rectObjectField,parameter.val_obj,typeof(OffMeshLink),bAllowSceneObject))) saveChanges = true; }
			else if(t == typeof(AudioListener)) { if(parameter.setObject(EditorGUI.ObjectField(rectObjectField,parameter.val_obj,typeof(AudioListener),bAllowSceneObject))) saveChanges = true; }
			else if(t == typeof(AudioSource)) { if(parameter.setObject(EditorGUI.ObjectField(rectObjectField,parameter.val_obj,typeof(AudioSource),bAllowSceneObject))) saveChanges = true; }
			else if(t == typeof(AudioReverbZone)) { if(parameter.setObject(EditorGUI.ObjectField(rectObjectField,parameter.val_obj,typeof(AudioReverbZone),bAllowSceneObject))) saveChanges = true; }
			else if(t == typeof(AudioLowPassFilter)) { if(parameter.setObject(EditorGUI.ObjectField(rectObjectField,parameter.val_obj,typeof(AudioLowPassFilter),bAllowSceneObject))) saveChanges = true; }
			else if(t == typeof(AudioHighPassFilter)) { if(parameter.setObject(EditorGUI.ObjectField(rectObjectField,parameter.val_obj,typeof(AudioHighPassFilter),bAllowSceneObject))) saveChanges = true; }
			else if(t == typeof(AudioEchoFilter)) { if(parameter.setObject(EditorGUI.ObjectField(rectObjectField,parameter.val_obj,typeof(AudioEchoFilter),bAllowSceneObject))) saveChanges = true; }
			else if(t == typeof(AudioDistortionFilter)) { if(parameter.setObject(EditorGUI.ObjectField(rectObjectField,parameter.val_obj,typeof(AudioDistortionFilter),bAllowSceneObject))) saveChanges = true; }
			else if(t == typeof(AudioReverbFilter)) { if(parameter.setObject(EditorGUI.ObjectField(rectObjectField,parameter.val_obj,typeof(AudioReverbFilter),bAllowSceneObject))) saveChanges = true; }
			else if(t == typeof(AudioChorusFilter)) { if(parameter.setObject(EditorGUI.ObjectField(rectObjectField,parameter.val_obj,typeof(AudioChorusFilter),bAllowSceneObject))) saveChanges = true; }
			else if(t == typeof(Camera)) { if(parameter.setObject(EditorGUI.ObjectField(rectObjectField,parameter.val_obj,typeof(Camera),bAllowSceneObject))) saveChanges = true; }
			else if(t == typeof(Skybox)) { if(parameter.setObject(EditorGUI.ObjectField(rectObjectField,parameter.val_obj,typeof(Skybox),bAllowSceneObject))) saveChanges = true; }
			// flare layer
			else if(t == typeof(GUILayer)) { if(parameter.setObject(EditorGUI.ObjectField(rectObjectField,parameter.val_obj,typeof(GUILayer),bAllowSceneObject))) saveChanges = true; }
			else if(t == typeof(Light)) { if(parameter.setObject(EditorGUI.ObjectField(rectObjectField,parameter.val_obj,typeof(Light),bAllowSceneObject))) saveChanges = true; }
			//else if(t == typeof(LightProbeGroup)) { if(parameter.setObject(EditorGUI.ObjectField(rectObjectField,parameter.val_obj,typeof(LightProbeGroup),bAllowSceneObject))) saveChanges = true; }
			else if(t == typeof(OcclusionArea)) { if(parameter.setObject(EditorGUI.ObjectField(rectObjectField,parameter.val_obj,typeof(OcclusionArea),bAllowSceneObject))) saveChanges = true; }
			//else if(t == typeof(OcclusionPortal)) { if(parameter.setObject(EditorGUI.ObjectField(rectObjectField,parameter.val_obj,typeof(OcclusionPortal),bAllowSceneObject))) saveChanges = true; }
			//else if(t == typeof(LODGroup)) { if(parameter.setObject(EditorGUI.ObjectField(rectObjectField,parameter.val_obj,typeof(LODGroup),bAllowSceneObject))) saveChanges = true; }
			else if(t == typeof(GUITexture)) { if(parameter.setObject(EditorGUI.ObjectField(rectObjectField,parameter.val_obj,typeof(GUITexture),bAllowSceneObject))) saveChanges = true; }
			else if(t == typeof(GUIText)) { if(parameter.setObject(EditorGUI.ObjectField(rectObjectField,parameter.val_obj,typeof(GUIText),bAllowSceneObject))) saveChanges = true; }
			else if(t == typeof(Animation)) { if(parameter.setObject(EditorGUI.ObjectField(rectObjectField,parameter.val_obj,typeof(Animation),bAllowSceneObject))) saveChanges = true; }
			//else if(t == typeof(NetworkView)) { if(parameter.setObject(EditorGUI.ObjectField(rectObjectField,parameter.val_obj,typeof(NetworkView),bAllowSceneObject))) saveChanges = true; }
			// wind zone
			else {
				
				if(t.BaseType == typeof(Behaviour))
				{ if(parameter.setObject(EditorGUI.ObjectField(rectObjectField,parameter.val_obj,typeof(Behaviour),bAllowSceneObject))) saveChanges = true; }
				else { if(parameter.setObject(EditorGUI.ObjectField(rectObjectField,parameter.val_obj,typeof(Component),bAllowSceneObject))) saveChanges = true; }
		    ...
		    //EditorGUIUtility.LookLikeControls();
            EditorGUIUtility.labelWidth=0f;
            if (parameter.setObject(EditorGUI.ObjectField(rectObjectField, parameter.val_obj, typeof(UnityEngine.Object),bAllowSceneObject))) saveChanges = true;
			GUI.skin = skin;
		    //EditorGUIUtility.LookLikeControls();
            EditorGUIUtility.labelWidth=0f;
		    ...
		    //EditorGUIUtility.LookLikeControls();
            EditorGUIUtility.labelWidth=0f;
            if (parameter.setObject(EditorGUI.ObjectField(rectObjectField, parameter.val_obj, typeof(Component),bAllowSceneObject))) saveChanges = true;
			GUI.skin = skin;
		    //EditorGUIUtility.LookLikeControls();
            EditorGUIUtility.labelWidth=0f;
		}else {
	...
	void showObjectFieldFor(AMTrack amTrack, float width_track, Rect rect) {
        ...
        // scale
        else if (amTrack is AMScaleTrack) {
            var strack = amTrack as AMScaleTrack;
            strack.obj = (Transform)EditorGUI.ObjectField(rect, strack.obj, typeof(Transform), true);
        }
        // animator
        else if (amTrack is AMAnimatorTrack)
        {
            var oneTrack = amTrack as AMAnimatorTrack;
            EditorGUI.BeginChangeCheck();
            Animator ator = (Animator)EditorGUI.ObjectField(rect, oneTrack.obj.SafeGetComponent<Animator>(), typeof(Animator), true);
            if (EditorGUI.EndChangeCheck())
            {
                oneTrack.obj = ator.transform;
            }
        }
        // animation
        ...
        else if (amTrack is AMPropertyTrack)
        {
            ...
            if ((amTrack as AMPropertyTrack).isObjectUnique(propertyGameObject))
            {
                ...
                if (changePropertyGameObject)
                {
                    ...
                    {
                        amTrack.deleteAllKeys(); //undo handled inside
                        amTrack.updateCache();
                        AMCodeView.refresh();
                    }

                    AMUtil.recordObject(amTrack, "Set GameObject");
	...
	void processDragLogic() {
		...
		if(justStartedDrag) {
			...
			if(mouseOverElement == (int)ElementType.Group || mouseOverElement == (int)ElementType.Track) {
                ...
                AMUtil.recordObject(this, "Move Element");
			...
			// if dragged from frame
				...
				if(mouseOverSelectedFrame) {
                    ...
                    AMUtil.recordObject(this, "Move Keys");
                    AMUtil.recordObject(aData.getCurrentTake(), "Move Keys");
					dragType = (int)DragType.MoveSelection;
					aData.getCurrentTake().setGhostSelection();
				} else {
                    //registerUndo("Select Frames");
                    AMUtil.recordObject(this, "Select Frames");
			...
			#region resize track
                ...
                AMUtil.recordObject(this, "Resize Track");
				startScrubMousePosition = currentMousePosition;
			...
			#region resize action
                ...
                AMUtil.recordObject(this, "Resize Action");
			...
			#region resize horizontal scrollbar left
                ...
                AMUtil.recordObject(this, "Resize Timeline");
    ...
    private float handDragAmount = 0f;
	void calculateNumFramesToRender(bool clickedZoom, Event e) {
		...
		if(dragType == (int)DragType.CursorHand && !justStartedHandGrab) {
			if(_mouseXOverFrame != startScrubFrame) {
                ...
                handDragAmount += (-e.delta.x / current_width_frame);
                float dist_hand_drag = Mathf.FloorToInt(handDragAmount);//startScrubFrame-_mouseXOverFrame;
                handDragAmount -= dist_hand_drag;
	...
	void timelineSelectTrack(int _track) {
        ...
        AMUtil.recordObject(aData.getCurrentTake(), "Select Track");
		// select track
	...
	void timelineSelectGroup(int group_id) {
        ...
        AMUtil.recordObject(aData.getCurrentTake(), "Select Group");
	...
	void timelineSelectObjectFor(AMTrack track) {
        ...
        else if (track.GetType() == typeof(AMAnimatorTrack))
            Selection.activeObject = (track as AMAnimatorTrack).obj;
        else if (track.GetType() == typeof(AMScaleTrack))
            Selection.activeObject = (track as AMScaleTrack).obj;
	}
	void timelineSelectNextKey(bool doWrap = true) 
    {
		// select next key
		if(aData.getCurrentTake().getTrackCount()<=0) return;
		...
		if(frame <= -1) return;
        if (!doWrap && frame < aData.getCurrentTake().selectedFrame) return;
		timelineSelectFrame(aData.getCurrentTake().selectedTrack,frame);
		
	}
    void timelineSelectPrevKey(bool doWrap = true)
    {
		// select previous key
		if(aData.getCurrentTake().getTrackCount()<=0) return;
		if(aData.getCurrentTake().getTrack(aData.getCurrentTake().selectedTrack).keys.Count <= 0) return;
	...
	string getInfoTextForAction(AMTrack _track, AMAction _action, bool brief, int clamped) {
		...
		#endregion
        #region animator
        } else if( _action is AMAnimatorAction ){
            if ((_action as AMAnimatorAction).Count == 0) return "Not Set";
            return "Key";
        #endregion
        #region audio
		...
		#region property
            ...
            if (!brief && (_action as AMPropertyAction).endFrame != -1 && (_action as AMPropertyAction).valueType != (int)AMPropertyTrack.ValueType.String)
            {
		...
		#endregion
        #region scale
        }else if (_action is AMScaleAction ){
            return easeTypeNames[(_action as AMScaleAction).easeType];
        #endregion
	...
	public Texture getTrackIconTexture(AMTrack _track) {
        ...
        else if (_track is AMAnimatorTrack) return texIconAnimator;
        ...
        else if (_track is AMScaleTrack) return texIconScale;
	...
	void addTrack(object trackType) {
        ...
        AMUtil.recordObject(this, "New Track");
        AMUtil.recordObject(aData.getCurrentTake(), "New Track");
	...
	void addTrackWithGameObject(object trackType, GameObject object_window) {
		...
		switch((int) trackType) {
			...
			case (int)Track.Orientation:
				...
				break;
            case (int)Track.Scale:
                aData.getCurrentTake().addScaleTrack(object_window);
                break;
            case (int)Track.Animator:
                aData.getCurrentTake().addAnimatorTrack(object_window);
                break;
	...
	void addKeyToFrame(int frame) {
        ...
        AMUtil.regUndoSelectedTrack(aData, "New Key");
    ...
    /// <summary>
    /// for each track in curTake.contextSelectionTracks, add key on the current frame
    /// </summary>
    bool addMultiKeysToSelectedFrame()
    {
        var curTake = aData.getCurrentTake();
        if (curTake == null)
            return false;

        bool bKeyAdded = false;
        var tracks = curTake.contextSelectionTracks;
        foreach (var trackId in tracks)
        {
            if (trackId < 0)
                continue;

            AMTrack track = curTake.getTrack(trackId);
            if (track == null)
                continue;

            bKeyAdded = true;
            AMUtil.recordObject(track, "New Key");

            // add a key
            addKey(trackId, curTake.selectedFrame);
        ...
        if (bKeyAdded)
        {
            ...
            timelineSelectFrame(curTake.selectedTrack, curTake.selectedFrame, false);
    ...
    /// <summary>
    /// add one key on current selectedTrack on selectedFrame
    /// </summary>
	bool addKeyToSelectedFrame() {

        AMUtil.regUndoSelectedTrack(aData, "New Key");
		// add key if there are tracks
        if (aData.getCurrentTake() && aData.getCurrentTake().getTrackCount() > 0 && aData.getCurrentTake().selectedTrack >= 0)
        {
            //registerUndo("New Key");
            // add a key
	...
	void addKey(int _track, int _frame) {
		...
		if(amTrack is AMTranslationTrack) {
            ...
            //add key to scale track
            sTrack.addKey(_frame, sTrack.obj.localScale);
        ...
        {
            // animator
            if (!(amTrack as AMAnimatorTrack).obj)
            {
                showAlertMissingObjectType("Animator");
                return;
            }
            // add key to animator track
            (amTrack as AMAnimatorTrack).addKey(_frame);
        }
        else if (amTrack is AMAudioTrack)
        {
            // audio
	...
	void deleteKeyFromSelectedFrame() {
        ...
        AMUtil.regUndoSelectedTrack(aData, "Clear Frame");
	...
	void deleteSelectedKeys(bool showWarning) {
		...
		if(shouldClearFrames) {
			foreach(int track_id in aData.getCurrentTake().contextSelectionTracks) {
				aData.getCurrentTake().deleteSelectedKeysFromTrack(track_id);	 //undo handled inside
			}
		}
	...
	void buildAddTrackMenu() {
        ...
        menu.AddItem(new GUIContent("Scale"), false, addTrackFromMenu, (int)Track.Scale);
        menu.AddItem(new GUIContent("Animator"), false, addTrackFromMenu, (int)Track.Animator);
	...
	void buildAddTrackMenu_Drag() {
		...
		foreach(GameObject g in objects_window) {
            ...
            if (hasAnimator && !g.GetComponent(typeof(Animator)))  {
                hasAnimator = false;
            }
        ...
        // Scale
        if (hasTransform) menu_drag.AddItem(new GUIContent("Scale"), false, addTrackFromMenu, (int)Track.Scale);
        else menu_drag.AddDisabledItem(new GUIContent("Scale"));
        // Animator
        if (hasAnimator) menu_drag.AddItem(new GUIContent("Animator"), false, addTrackFromMenu, (int)Track.Animator);
        else menu_drag.AddDisabledItem(new GUIContent("Animator"));
		// Animation
	...
	void buildContextMenu(int frame) {
        ...
        bool selectionHasKeys;
        bool canPaste;
        _VerifySelectionAndPaste(out selectionHasKeys, out canPaste);
	...
	void contextCutKeys() {
        ...
        AMUtil.recordObject(this, "Cut Keys");
		contextCopyFrames();
	...
	void contextPasteKeys() {
        ...
        AMUtil.recordObject(this, "Paste keys");        
		...
		if(singleTrack) {
            ...
            AMUtil.recordObject(aData.getCurrentTake().getSelectedTrack(), "Paste keys");
			foreach(AMKey a in contextSelectionKeysBuffer[0]) {
				// offset keys based on selection range
				a.frame += (contextMenuFrame-(int)contextSelectionRange.x); //I don't use undo to handle this
			...
			for(int i=0;i<contextSelectionTracksBuffer.Count;i++) {
                ...
                {
                    AMUtil.recordObject(contextSelectionTracksBuffer[i], "Paste keys");
                    // offset keys based on selection range
		...
		checkForOutOfBoundsFramesOnSelectedTrack();
        AMUtil.regUndoSelectedTake(aData, "Paste keys");
        // update cache
		if(singleTrack) {
			aData.getCurrentTake().getSelectedTrack().updateCache();
		} else {
	...
	void contextSelectAllFrames() {
        ...
        AMUtil.regUndoSelectedTake(aData, "Select All Frames");
	...
	void cancelTextEditting(bool toggleIsRenamingTake = false) {
		...
		if(isRenamingTake) {
            var curTake = aData.getCurrentTake();
			aData.makeTakeNameUnique(curTake);
            AssetDatabase.RenameAsset(AssetDatabase.GetAssetPath(curTake), curTake.name);
            Repaint();
		}
    ...
    private void _OnKeyboardEvent(Event e)
    {
        _KeyboardShortcut();
        ...
        if (e.type == EventType.MouseDown && e.button == 2) isMMBDown = true;
        else if (e.rawType == EventType.MouseUp && e.button == 2) isMMBDown = false;
    }

    private void _KeyboardShortcut()
    {
        Event e = Event.current;

        if ( EditorGUIUtility.editingTextField || isRenamingTake || isRenamingTrack != -1 || isRenamingGroup != 0) //when textField is activated, do not accept shortcut
            return;
            ...
            case EventType.ExecuteCommand:
                {
                    ...
                    {
                        if (_ThisWndHasFocus()) //ctrl-V when has focus
                        {
                            var selFrame = aData.getCurrentTake().selectedFrame;
                            if (selFrame >= 0)
                            {
                                bool canPaste = _VerifyCanPaste();
                                if (canPaste)
                                {
                                    contextMenuFrame = selFrame;
                                    contextPasteKeys();
                                    this.Repaint();
                                    e.Use();
                                }                                
                    ...
                    {
                        if (_ThisWndHasFocus() ) //ctrl-X when has focus
                        {
                            contextCutKeys();
                            this.Repaint();
                            e.Use();
                    ...
                    {
                        if (_ThisWndHasFocus() ) //ctrl-A when has focus
                        {
                            contextSelectAllFrames();
                            this.Repaint();
                            e.Use();
                        }
                    }
            ...
            case EventType.KeyUp:
                {
                    switch (e.keyCode)
                    {
                        ...
                        case KeyCode.DownArrow:
                            {
                                var curTake = aData.getCurrentTake();
                                if (_ThisWndHasFocus() && curTake.selectedTrack >= 0 && curTake.getTrackCount() > 1)
                                {
                                    List<int> track_ids = new List<int>();
                                    curTake.getTrackIDsForGroup(0, track_ids);

                                    int delta = e.keyCode == KeyCode.DownArrow ? 1 : track_ids.Count - 1;

                                    int newIdx = (track_ids.IndexOf(curTake.selectedTrack) + delta) % track_ids.Count;
                                    int newTrackId = track_ids[newIdx];

                                    curTake.selectTrack(newTrackId, false, false);
                                    Repaint();
                                }
                            }
                            break;
            ...
            case EventType.KeyDown:
                {
                    switch (e.keyCode)
                    {
                        ...
                        case KeyCode.End:
                            {
                                //if (_ThisWndHasFocus()){
                                    _SetSelectedFrame(aData.getCurrentTake().numFrames);
                                    this.Repaint();
                                    e.Use();
                                //}                                
                            }
                        ...
                        case KeyCode.RightArrow:
                            {
                                ...
                                {
                                    int f;
                                    int offset = (e.keyCode == KeyCode.LeftArrow ? -1 : 1);

                                    if (e.alt)
                                        offset *= 5;
                                    f = Mathf.Clamp(curTake.selectedFrame + offset, 1, curTake.numFrames + 1);

                                    _SetSelectedFrame(f);
                                }
                                else 
                                {
                                    if (curTake.selectedTrack >= 0)
                                    {
                                        if (e.keyCode == KeyCode.LeftArrow)
                                            timelineSelectPrevKey(false);
                                        else
                                            timelineSelectNextKey(false);
                                    }
                                }
                        ...
                        case KeyCode.I:
                            {
                                //int grpId = AMUtil.prepUndoGrp("Add Key");

                                bool bSuccess = addMultiKeysToSelectedFrame();
                                if( bSuccess)
                                {
                                    this.Repaint();
                                    e.Use();
                        ...
                        case KeyCode.Delete:
                            {
                                ...
                                {
                                    var curTake = aData.getCurrentTake();
                                    if (curTake && curTake.track_count > 0)
                                    {
                                        if (curTake.hasContextSelection())
                                        {//delete keys
                                            foreach (int trackId in curTake.contextSelectionTracks)
                                            {
                                                if( trackId >= 0)
                                                    curTake.deleteSelectedKeysFromTrack(trackId);
                                            }
                                            this.Repaint();
                                        }
                                        else if( curTake.hasContextSelectionTracks() )
                                        {//delete tracks
                                            _DeleteTrackWithConfirm();
                                            this.Repaint();
                                        }
                                        else if (curTake.selectedGroup < 0)
                                        {
                                            _DeleteGroupWithConfirm();
                                            this.Repaint();
                                        }
                                        e.Use();
                                    }
    ...
    private void _SetSelectedFrame(int f)
    {
        var curTake = aData.getCurrentTake();
        curTake.selectedFrame = f;
        if (curTake.selectedFrame < curTake.startFrame)
        {
            curTake.startFrame = curTake.selectedFrame;
            curTake.endFrame = curTake.startFrame + numFramesToRender - 1;
        }
        else if (curTake.selectedFrame > curTake.endFrame)
        {
            curTake.endFrame = curTake.selectedFrame;
            curTake.startFrame = curTake.endFrame - numFramesToRender + 1;
        }
        curTake.previewFrame(f);
    }

    private bool _ThisWndHasFocus()
    {
        return EditorWindow.focusedWindow == this;
    }

    private void _OnClickWindowBG()
    {
        if (GUI.Button(new Rect(0f, 0f, position.width, position.height), "", "label") && dragType != (int)DragType.TimelineScrub && dragType != (int)DragType.ResizeAction)
        {
            ...
            if (aData.getCurrentTake().contextSelectionTracks != null && aData.getCurrentTake().contextSelectionTracks.Count > 0)
            {
                ...
                AMUtil.regUndoSelectedTake(aData, "Deselect Tracks");

                didRegisterUndo = true;
                var tracks = aData.getCurrentTake().contextSelectionTracks;
                tracks.Clear();
                if (aData.getCurrentTake().selectedTrack != -1)
                    tracks.Add(aData.getCurrentTake().selectedTrack);
            }
            if (aData.getCurrentTake().contextSelection != null && aData.getCurrentTake().contextSelection.Count > 0)
            {
                ...
                {
                    //registerUndo("Deselect Frames");
                    AMUtil.regUndoSelectedTake(aData, "Deselect Frames");
                }
                didRegisterUndo = true;
                aData.getCurrentTake().contextSelection.Clear();
            }
            if (aData.getCurrentTake().ghostSelection != null && aData.getCurrentTake().ghostSelection.Count > 0)
            {
                aData.getCurrentTake().ghostSelection.Clear();
            }

            if (objects_window.Count > 0) objects_window.Clear();
            ...
            // if click on timeline, jump to that frame
            if (mouseOverElement == (int)ElementType.TimelineScrub)
                _SetSelectedFrame(mouseXOverFrame);
        }
    }

    private void _OnClickDeleteTrackBtn()
    {
        cancelTextEditting();
    ...
    private void _DeleteGroupWithConfirm()
    {
        if (aData.getCurrentTake().selectedGroup >= 0) //cannot delete rootGroup (0)
            return;
        ...
        if (grp.elements.Count > 0)
        {
            ...
            if (delete)
            {
                ...
                aData.getCurrentTake().deleteSelectedGroup(deleteContents);//undo handled inside
                AMCodeView.refresh();
            }
        }
        else
    ...
    private void _DeleteTrackWithConfirm()
    {
        ...
        if ((EditorUtility.DisplayDialog("Delete Track", "Are you sure you want to delete " + strMsgDeleteTrack + "?", "Delete", "Cancel")))
        {
            ...
            foreach (int track_id in aData.getCurrentTake().contextSelectionTracks)
            {
                aData.getCurrentTake().deleteTrack(track_id); //undo handled inside
            }
            AMUtil.recordObject(aData.getCurrentTake(), "delete track");
    ...
    private bool _VerifyCanPaste()
    {
        bool selectionHasKeys, canPaste;
        _VerifySelectionAndPaste(out selectionHasKeys, out canPaste);
        return canPaste;
    }

    private void _VerifySelectionAndPaste(out bool selectionHasKeys, out bool canPaste)
    {
        selectionHasKeys = canPaste = false;
        if (aData.getCurrentTake().selectedTrack < 0)
            return;

        selectionHasKeys = aData.getCurrentTake().contextSelectionTracks.Count > 1 || aData.getCurrentTake().contextSelectionHasKeys();
        ...
        if (copyBufferNotEmpty)
        {
            if (singleTrack)
            {
                ...
                else if (selectedTrack is AMEventTrack)
                {
                    ...
                    {
                        // if event tracks are compatible
    ...
    private void _OnTooltip(Event e)
    {
    ...
    private void _OnBeforeChangeCurTake(AMTake curTake, AMTake newTake)
    {
        if (curTake != null)
        {
            curTake.SaveAsset(aData);
        }
        if (newTake != null && !EditorUtility.IsPersistent(newTake))
        {
            newTake.SaveAsset(aData);
        }
    }

    private bool _OnGUI_NoTake()
    {
        if (aData.takes.Count == 0)
        {
            ...
            float h = position.height * 0.4f;
            GUILayout.BeginHorizontal(GUILayout.Height(h));
            {
                GUILayout.Space(position.width * 0.1f);
                if (GUILayout.Button("Create New Take", btnStyle, GUILayout.Width(position.width * 0.35f), GUILayout.Height(h)))
                {
                    _ShowCreateNewTakeWindow();
                }
                GUILayout.Space(position.width * 0.1f);
                Rect rc = GUILayoutUtility.GetRect(position.width * 0.35f, h);
                if (GUI.Button(rc, "Import Existing Take", btnStyle))
                {
                    _ShowImportTakeWindow(rc);
                }
                GUILayout.Space(position.width * 0.1f);
            }
            GUILayout.EndHorizontal();
        ...
        return aData.takes.Count != 0;
    }

    private void _ShowCreateNewTakeWindow()
    {
        string newPath = EditorUtility.SaveFilePanelInProject("Create new Take", "NewTake", "asset", "select new take location");
        if (!string.IsNullOrEmpty(newPath))
        {
            string basename = Path.GetFileNameWithoutExtension(newPath);
            aData.addAndSaveTake(basename, newPath);
            //var curTake = aData.getCurrentTake();
            //curTake.setNameByProjPath(newPath);
            //curTake.SaveAsset(aData, newPath);
            EUtil.ShowNotification("Created New Take at: " + newPath);
        }
    }

    private void _ShowImportTakeWindow(Rect refRc)
    {
        PopupWindow.Show(refRc, new SelectObjWindow<AMTake>(
            x =>
            {
                if (x == null) return;
                aData.importTake((AMTake)x);
                AMCodeView.resetTrackDictionary();
                // save data
                EditorUtility.SetDirty(aData);
                setDirtyTakes(aData.takes);
            ...
            "Select one Take to import:"
        ));
    }

    private void _SetDataHolderGO(GameObject go)
    {
        ms_dataHolderGO = go;
        m_dataHolderGO = go;
    }

    private GameObject _GetDataHolderGO()
    {
        if (ms_dataHolderGO != null)
        {
            m_dataHolderGO = ms_dataHolderGO;
        }
        else
        {
            ms_dataHolderGO = m_dataHolderGO;
        }
        return ms_dataHolderGO;
    }
    ...
    private void _LoadRes()
    {
         ...
         tex_icon_track_hover = (Texture)Resources.Load("am_icon_track_hover");
         tex_icon_group_closed = (Texture)Resources.Load("am_icon_group_closed");
         tex_icon_group_open = (Texture)Resources.Load("am_icon_group_open");
         tex_icon_group_hover = (Texture)Resources.Load("am_icon_group_hover");
        ...
        // texFrU = (Texture)Resources.Load("am_frame");
        // texFrM = (Texture)Resources.Load("am_frame-m");
        // texFrUS = (Texture)Resources.Load("am_frame-s"); 
        // texFrMS = (Texture)Resources.Load("am_frame-m-s"); 
        // texFrUG = (Texture)Resources.Load("am_frame-g"); 
         ...
         texProperties = (Texture)Resources.Load("am_information");
         ...
         texInterpl = new Texture[]{ (Texture)Resources.Load("am_interpl_curve"), (Texture)Resources.Load("am_interpl_linear") };
         texBoxBorder = (Texture)Resources.Load("am_box_border");
         texBoxRed = (Texture)Resources.Load("am_box_red");
         texBoxBlue = (Texture)Resources.Load("am_box_blue");
         ...
         texIconAnimator = (Texture)Resources.Load("am_icon_animator");
         ...
         texIconScale = (Texture)Resources.Load("am_icon_scale");
         texIconCameraSwitcher = (Texture)Resources.Load("am_icon_cameraswitcher");

         texTakeIcon = (Texture)AssetDatabase.LoadAssetAtPath(AnimatorData.BASEDIR + "/Files/Editor/Res/am_timelineTitleIcon.psd", typeof(Texture));
    }

	#region "undo modification processing"

    private bool m_inPostProcessMod = false;
    private UndoPropertyModification[] _OnPostprocessModifications(UndoPropertyModification[] modifications)
    {
        if( !aData || !aData.autoKey )
            return modifications;
        var curTake = aData.getCurrentTake();
        if (curTake == null)
            return modifications;
        if (m_inPostProcessMod)
            return modifications;

        m_inPostProcessMod = true;

        bool bModified = false;
        // create tracks or insert keys or modify keys
        for (int i = 0; i < modifications.Length; ++i)
        {
            //var mod = modifications[i].propertyModification;
            var mod = EUtil.GetPropertyModification(modifications[i]);
            if (_IsValidMod(mod) )
            {

                AMTrack amTrack = _EnsureTrackForMod(mod);

                AMUtil.recordObject(amTrack, "New Key");

                // add a key
                addKey(amTrack.id, curTake.selectedFrame);

                // save data
                setDirtyKeys(amTrack);
                setDirtyCache(amTrack);
        ...
        m_inPostProcessMod = false;

        return modifications;
    }

    private AMTrack _EnsureTrackForMod(PropertyModification mod)
    {
        var curTake = aData.getCurrentTake();
        Transform targetTr = (Transform)mod.target;

        Track trackType = _GetModTrackType(mod);

        foreach (AMTrack track in curTake.trackValues)
        {
            switch( trackType )
            {
                case Track.Translation:
                    {
                        ...
                        {
                            var transTrack = (AMTranslationTrack)track;
                            if (transTrack.obj == targetTr)
                                return transTrack;                            
                        }
                    }
                    break;
                case Track.Rotation:
                    {
                        if (track is AMRotationTrack)
                        {
                            var rotTrack = (AMRotationTrack)track;
                            if (rotTrack.obj == targetTr)
                                return rotTrack;
                        }
                    }
                    break;
                case Track.Scale:
                    {
                        if (track is AMScaleTrack)
                        {
                            var scaleTrack = (AMScaleTrack)track;
                            if (scaleTrack.obj == targetTr)
                                return scaleTrack;
                        }
                    }
                    break;
        ...
        addTrackWithGameObject(trackType, targetTr.gameObject);
        AMTrack newTrack = curTake.trackValues[curTake.trackValues.Count-1];
        return newTrack;
    }

    private Track _GetModTrackType(PropertyModification mod)
    {
        Track tool = Track.None;
        if (mod.target is Transform)
        {
            if (mod.propertyPath.StartsWith("m_LocalPosition"))
                tool = Track.Translation;
            else if (mod.propertyPath.StartsWith("m_LocalRotation"))
                tool = Track.Rotation;
            //else if (mod.propertyPath.StartsWith("m_LocalScale"))
                //tool = Tool.Scale;
        }        
        return tool;
    }

    private bool _IsValidMod(PropertyModification mod)
    {
        Track tool = _GetModTrackType(mod);
        return tool == Track.Translation || tool == Track.Rotation || tool == Track.Scale;
    }

    private void _RebindCurrentTake()
    {
        if (aData != null && aData.takes.Count > 0)
        {
            if (aData.currentTake >= 0)
            {
                var curTake = aData.getCurrentTake();
                curTake.rebind(aData.rebindOption);
            }
        }        
    }

	#endregion "undo modification processing"

	#region "traverse take tracks"
	

	
	#endregion "traverse take tracks"
