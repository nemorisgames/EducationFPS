...
namespace CBG.FPSMeshTool {
    ...
    public class FPSMeshToolWindow : EditorWindow {
            ...
            if (!validObject) {
                if (isPrefab) {
                    ...
                    if (GUILayout.Button("Instantiate")) {
                        ...
                        Undo.RecordObject(go, "Renamed " + go.name + " to " + sourceObject.name);
        ...
        void GuessBones(GameObject sourceObject) {
            Undo.RecordObject(this, "Changed bones");
            if (validObject && validRenderers) {
                rootBone = GetRootBone(sourceRenderers);
                if (validRootBone) {
        ...
        void BuildFPSMesh() {
            ...
            PrefabUtility.CreatePrefab(GetValidAssetName(GeneratePrefabFileName(previewInstance)), previewInstance);
        ...
        void AddMaterial(List<Material> materials, List<string> materialNames, Material mat, string suffix) {
            if (mat == null) {
        ...
        void ConvertToFPSMesh(SkinnedMeshRenderer sourceRenderer, SkinnedMeshRenderer targetRenderer, Transform head, Transform leftArm, Transform rightArm, bool processLegs, Transform leftLeg, Transform rightLeg) {
            ...
            //newMesh.RecalculateBounds(); // excluded because it was messing up meshes
            //newMesh.RecalculateNormals(); // excluded because it was messing up meshes
        ...
        void UpdatePreview() {
            ...
            previewInstance = (GameObject)GameObject.Instantiate(sourceObject, pos, rot);
            Undo.RegisterCreatedObjectUndo(previewInstance, "Created previewInstance");
            Undo.RecordObject(previewInstance, "renamed previewInstance");
