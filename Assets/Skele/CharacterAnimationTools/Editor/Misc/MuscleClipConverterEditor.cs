...
public class MuscleClipConverterEditor : EditorWindow
{
    ...
    private EMode m_Mode = EMode.Standard;
    ...
    public static void OpenWindow()
    {
        if( ms_Instance == null )
        {
            ...
            if( Selection.activeGameObject )
                inst.m_Animator = Selection.activeGameObject.GetComponentInParent<Animator>(); //init animator field if possible

            inst._questionMark = AssetDatabase.LoadAssetAtPath(MH.Skele.CAT.ResPath.QuestionMark, typeof(Texture2D)) as Texture2D;
        }        
    }
    ...
    void OnGUI()
    {
        ...
        m_Mode = (EMode)EditorGUILayout.EnumPopup("Conversion Mode", m_Mode);
        ...
        if( EUtil.Button("Convert Animation!", bSet ? Color.green : Color.red) )
        {
            ...
            else
            {
                ...
                string newClipAssetPath = PathUtil.StripExtension(oldClipAssetPath) + NEW_CLIP_POSTFIX + m_AnimType + m_Mode + ".anim";
                ...
                if (filePath.Length > 0)
                {
                    ...
                    if (m_Mode == EMode.RootMotion)
                        _ConvertAnim_RootMotion(filePath);
                    else if (m_Mode == EMode.Standard)
                        _ConvertAnim_Standard(filePath);
                    else
                        Dbg.LogErr("Unexpected mode: {0}", m_Mode);                    
                }
                else
                {
        ...
        {
            Application.OpenURL("http://www.tmpxyz.com/Skele/docs/84-utility/80-utilities?showall=&start=2");
        }
    }
    ...
    private void _ConvertAnim_Standard(string newClipAssetPath)
    {
        // 0. prepare
        if (!m_Animator.isHuman)
        {
            Dbg.LogWarn("MuscleClipConverterEditor._ConvertAnim_Standard: Need to change to Humanoid rig first!");
            return;
        }
        ...
        if (m_SMR == null)
        {
            Dbg.LogWarn("MuscleClipConverterEditor._ConvertAnim_Standard: failed to find SMR under {0}", m_Animator.name);
            return;
        }
        ...
        clipSetting.loopBlend = false; //if this is true, the result animation will walk unmoved (in-place)
        ...
        bool isUseApplyRootmotion = m_Animator.applyRootMotion;
        m_Animator.applyRootMotion = true; //if not apply rootmotion, the character will walk in-place, the generated clip will be in-place in the end
        ...
        Vector3 animatorOrigPos = animatorTr.position;
        Quaternion animatorOrigRot = animatorTr.rotation;
        ...
        { // 1. bake animation info into curve on all bones transform
                ...
                )
            {
                ...
                for (int idx = 0; idx < bones.Length; ++idx)
                {
                    ...
                    if (oneBone == hipsBone)
                        continue; //skip the HipsBone. hips is handled separately
                ...
                // root motion process
                {
                    { //on hips transform
                        ...
                        {
                            ///------------------cache--------------------///
                            Vector3 animatorCurPos = animatorTr.position;
                            Quaternion animatorCurRot = animatorTr.rotation;
                            Vector3 hipCurPos = hipsBone.position;
                            Quaternion hipCurRot = hipsBone.rotation;

                            ///------------------move animatorTr back to original but keep hips at current pos--------------------///
                            animatorTr.position = animatorOrigPos;
                            animatorTr.rotation = animatorOrigRot;
                            hipsBone.position = hipCurPos;
                            hipsBone.rotation = hipCurRot;

                            ///------------------prepare write curve--------------------///
                            _Curves hipsCurves = curveDict[hipsBone];
                            Vector3 lpos = hipsBone.localPosition;
                            Quaternion lrot = hipsBone.localRotation;

                            //Dbg.Log("time: {0}, lpos: {1}, hipsBonePos: {2}", time, lpos.ToString("F2"), hipsBone.position.ToString("F2"));

                            hipsCurves.X.AddKey(time, lrot.x);
                            hipsCurves.Y.AddKey(time, lrot.y);
                            hipsCurves.Z.AddKey(time, lrot.z);
                            hipsCurves.W.AddKey(time, lrot.w);

                            hipsCurves.PX.AddKey(time, lpos.x);
                            hipsCurves.PY.AddKey(time, lpos.y);
                            hipsCurves.PZ.AddKey(time, lpos.z);

                            ///------------------revert animatorTr back to current pos--------------------///
                            animatorTr.position = animatorCurPos;
                            animatorTr.rotation = animatorCurRot;
                            hipsBone.position = hipCurPos;
                            hipsBone.rotation = hipCurRot;
                        }
                    }
        ...
        m_Animator.applyRootMotion = isUseApplyRootmotion;

        Dbg.Log("Converted: {0}", newClipAssetPath);
    }

    private void _ConvertAnim_RootMotion(string newClipAssetPath)
    {
        // 0. prepare
        if( !m_Animator.isHuman )
        {
            Dbg.LogWarn("MuscleClipConverterEditor._ConvertAnim_RootMotion: Need to change to Humanoid rig first!");
            return;
        }
        ...
        if( m_SMR == null )
        {
            Dbg.LogWarn("MuscleClipConverterEditor._ConvertAnim_RootMotion: failed to find SMR under {0}", m_Animator.name);
            return;
        }
        ...
        //Vector3 animatorOrigPos = animatorTr.localPosition;
        //Quaternion animatorOrigRot = animatorTr.localRotation;
        ...
        // init curve for root(animatorTr)
        {
            var curves = new _Curves();
            curves.relPath = string.Empty;
        ...
        { // 1. bake animation info into curve on all bones transform
                ...
                )
            {
                ...
                // root motion process
                {
                    _Curves rootCurves = curveDict[animatorTr];
                    Vector3 lpos = animatorTr.localPosition;
                    Quaternion lrot = animatorTr.localRotation;

                    rootCurves.X.AddKey(time, lrot.x);
                    rootCurves.Y.AddKey(time, lrot.y);
                    rootCurves.Z.AddKey(time, lrot.z);
                    rootCurves.W.AddKey(time, lrot.w);

                    rootCurves.PX.AddKey(time, lpos.x);
                    rootCurves.PY.AddKey(time, lpos.y);
                    rootCurves.PZ.AddKey(time, lpos.z);
                }
        ...
        { // 2. set animation clip and store in AssetDatabase
            ...
            if (m_AnimType == ModelImporterAnimationType.Generic)//generate rootmotion
                RCall.CallMtd("UnityEditor.AnimationUtility", "SetGenerateMotionCurves", null, newClip, true);
    ...
    ///// <summary>
    ///// the old implementation, backup for reference
    ///// </summary>
    ...
    public const string NEW_CLIP_POSTFIX = "_";
