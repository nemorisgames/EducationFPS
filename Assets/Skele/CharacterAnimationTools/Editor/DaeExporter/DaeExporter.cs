...
namespace MH
{
    ...
    public class DaeExporter
    {
        ...
        private List<AnimationClip> m_ClipsWithRootCurves = new List<AnimationClip>();
        ...
        private bool m_IgnoreRootCurves = false;

        // debug usage
        private bool m_Debug = false;
        private StreamWriter m_DbgSW;

        public bool ignoreRootCurves { get { return m_IgnoreRootCurves; } set { m_IgnoreRootCurves = value; } }
        public bool hasRootCurves { get { return m_ClipsWithRootCurves.Count > 0; } }
        public bool shouldMakeExtraRoot { get { return hasRootCurves && !ignoreRootCurves; } }
        ...
        public void Export(List<AnimationClip> clips, string outPath = "export.dae")
        {
            ...
            FindClipsWithRootCurves(m_Clips, m_ClipsWithRootCurves);
        ...
        public static bool FindClipsWithRootCurves(List<AnimationClip> src, List<AnimationClip> dst)
        {
            dst.Clear();
        ...
        private void _PrepareToExportAsHierarchyNodeList()
        {
            ...
            {
                if (shouldMakeExtraRoot)
                    m_NeedExportHierarchyNodes.Add(m_AnimRoot);
        ...
        private void _DebugInvBindInfo(SkinnedMeshRenderer smr, int i, ref Matrix4x4 m)
        {
            ...
            Dbg.Log("InvBind: SMR: {0}, bone:{1}, pos:{2:F2}, z:{3:F2}, y:{4:F2}, x:{5:F2}", smr.name, bone.name, pos, z, y, x);
        }
        ...
        private static Matrix4x4 _ReflectXMatrix(Matrix4x4 m)
        {
            ...
            bool isLeftHandness = Misc.IsAcuteAngle(vZ, Vector3.Cross(vX, vY)); //we need to handle negative scale, which is right-handness

            Vector3 newVZ = Vector3.Reflect(vZ, Vector3.right);
            Vector3 newVY = Vector3.Reflect(vY, Vector3.right);
            Vector3 newVX;
            if (isLeftHandness)
                newVX = Vector3.Cross(newVY, newVZ).normalized * xmag;
            else
                newVX = Vector3.Cross(newVY, newVZ).normalized * xmag * -1f;
